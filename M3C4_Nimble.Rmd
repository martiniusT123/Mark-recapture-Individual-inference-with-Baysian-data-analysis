...
index.na <- which(index$na[,3]>(N-501))[1]
index.observe <- which(index$observe[,3]>(N-501))[1]

cov.na.500 <- index$na[index.na:dim(index$na)[1],]
cov.observe.500 <- index$observe[index.observe:dim(index$observe)[1],]
...
#                   zeroth.cohort=76,
                   ...
                   na.index=dim(cov.na.500)[1],
                   X.na=cov.na.500,
                   
                   na.sex.index=length(sex.na.500),
                   sex.na=sex.na.500)
### 1000 different                   
...
params <- c("rho","varians",                #Density approximation of precision
            ...
            "sex.prob")      #Prior for sex hyperparameter
...
model.hidden <- nimbleCode({
  ...
  for(index in 1:na.index){#Censoring of X.covatiates
    x[X.na[index,1],X.na[index,2],X.na[index,3]] ~ dnorm(0,1)
  }

  for(index in 1:na.sex.index){ #Handeling of censoring of sex
